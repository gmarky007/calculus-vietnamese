...
results = []
for p in range(94, 112):
    fpath = os.path.join(PAGES_DIR, f'page_{p:04d}.tex')
    if not os.path.exists(fpath):
        results.append((p, 'MISSING_FILE'))
        print(f"Trang {p}: MISSING_FILE", flush=True)
        continue
...
print(f"\n=== KET QUA KIEM TRA BATCH 3 (TRANG 94-111): {ok_count}/{len(results)} TRANG DAT 1 TRANG ===", flush=True)
