...
# Pre-existing 99% pages from Section 1.1 test
EXISTING_PAGES = {
    43: r"""\fancyhead[L]{\textbf{\large 8}\quad\sffamily\textbf{\color{darkgray}CHƯƠNG 1}\quad\sffamily\color{darkgray}Hàm số và Mô hình}
\fancyhead[R]{}
...
\begin{minipage}[t]{0.26\textwidth}
    \vspace{1.5cm}
    \includegraphics[width=\linewidth]{images/figure_2_clean.png}\\[2pt]
    {\small\textbf{\textsf{HÌNH 2}}}\\[1pt]
    {\footnotesize Sơ đồ cỗ máy cho hàm số $f$}

    \vspace{1.6cm}
    \includegraphics[width=\linewidth]{images/figure_3_clean.png}\\[2pt]
    {\small\textbf{\textsf{HÌNH 3}}}\\[1pt]
    {\footnotesize Sơ đồ mũi tên cho $f$}

    \vspace{5.8cm}
    \includegraphics[width=\linewidth]{images/figure_6_clean.png}\\[2pt]
...
\begin{minipage}[t]{0.71\textwidth}
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=0.92\linewidth]{images/figure_4_clean.png}\\[2pt]
        {\small\textbf{\textsf{HÌNH 4}}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=0.92\linewidth]{images/figure_5_clean.png}\\[2pt]
...
\end{minipage}
""",
    45: r"""\fancyhead[L]{\textbf{\large 10}\quad\sffamily\textbf{\color{darkgray}CHƯƠNG 1}\quad\sffamily\color{darkgray}Hàm số và Mô hình}
\fancyhead[R]{}
...
\begin{minipage}[t]{0.71\textwidth}
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{images/figure_9_clean.png}\\[2pt]
        {\small\textbf{\textsf{HÌNH 9}}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{images/figure_10_clean.png}\\[2pt]
...
\end{minipage}
""",
    47: r"""\fancyhead[L]{\textbf{\large 12}\quad\sffamily\textbf{\color{darkgray}CHƯƠNG 1}\quad\sffamily\color{darkgray}Hàm số và Mô hình}
\fancyhead[R]{}
...
def call_vision_llm(page_num, b64_img, available_figs):
    ...
    prompt = f"""Bạn là một chuyên gia số hóa sách giáo trình quốc tế hàng đầu.
Nhiệm vụ: Chuyển đổi trang {page_num} của sách "Calculus: Early Transcendentals (9th Edition)" của James Stewart sang mã nguồn LaTeX tương thích với template 2 cột bất đối xứng của sách.

QUY TẮC BẮT BUỘC:
1. BỐ CỤC 2 CỘT:
Mỗi trang BẮT BUỘC phải nằm trọn vẹn trong cấu trúc sau:
\\fancyhead[L]{{...}} \\fancyhead[R]{{...}}
\\noindent
\\begin{{minipage}}[t]{{0.26\\textwidth}}
    % Cột lề: chứa bảng nhỏ, hình nhỏ, ghi chú bên lề
\\end{{minipage}}%
\\hfill
\\begin{{minipage}}[t]{{0.71\\textwidth}}
    % Cột chính: tiêu đề, nội dung lý thuyết, ví dụ, công thức, bảng/hình lớn
\\end{{minipage}}

2. TOÁN HỌC & MÀU SẮC:
- Dùng màu \\color{{stewartcyan}} cho tiêu đề mục, ví dụ (\\textbf{{\\textsf{{\\color{{stewartcyan}}VÍ DỤ X}}}}), lời giải.
- Khung định nghĩa: dùng \\begin{{definitionbox}} ... \\end{{definitionbox}} (viền đỏ stewartred).
- Kết thúc lời giải: đặt ô vuông cyan {{\\color{{stewartcyan}}\\blacksquare}} (trong toán) hoặc {{\\color{{stewartcyan}}$\\blacksquare$}} (ngoài toán).
- Các hình khả dụng trên trang này đã crop: {figs_info}. Chỉ chèn hình nếu đúng vị trí.
- Không bao bọc trong ```latex ... ```, chỉ trả về đoạn mã LaTeX hoàn chỉnh của trang.
"""
    payload = {
        "model": MODEL,
        "messages": [
...
"""
    ...
    print(f"--> Đang biên dịch chapter_01.pdf ({len(completed)} trang đã hoàn thành)...")
...
def main():
    # Chapter 1 ranges from PDF page 42 to 111 (70 pages)
    START_PAGE = 42
    END_PAGE = 111
    ...
    # Ensure existing verified pages are saved
    for p_num, content in EXISTING_PAGES.items():
        p_file = os.path.join(PAGES_DIR, f"page_{p_num:04d}.tex")
        if not os.path.exists(p_file):
            with open(p_file, "w", encoding="utf-8") as f:
                f.write(content)
        if p_num not in cp["completed_pages"]:
            cp["completed_pages"].append(p_num)
    save_checkpoint(cp)
    
    print(f"=== KHỞI CHẠY TỰ ĐỘNG CHƯƠNG 1 (Trang {START_PAGE} đến {END_PAGE}) ===")
    print(f"Trang đã hoàn thành sẵn: {sorted(cp['completed_pages'])}\n")
    
    for page_num in range(START_PAGE, END_PAGE + 1):
        if page_num in cp["completed_pages"]:
            continue
            
        print(f"--> [Trang {page_num}/{END_PAGE}] Đang xử lý...")
        page_idx = page_num - 1
        ...
        if tex_content:
            ...
            save_checkpoint(cp)
            print(f"✓ Hoàn thành trang {page_num} ({len(available_figs)} hình)")
        else:
            print(f"✗ Thất bại ở trang {page_num}")
            
        time.sleep(1.5)
        
        # Compile checkpoint every 5 pages
        if len(cp["completed_pages"]) % 5 == 0:
            compile_master_pdf()
            
    doc.close()
    compile_master_pdf()
    print("\n🎉 HOÀN TẤT TOÀN BỘ CHƯƠNG 1!")
