...
\usepackage{geometry}
\geometry{top=1.6cm, bottom=1.5cm, left=1.8cm, right=1.8cm, headheight=14pt, headsep=10pt}
\usepackage{xcolor}
\usepackage{tikz}
\usepackage{enumitem}
\usepackage{fancyhdr}
...
% Màu chuẩn sách giáo trình James Stewart Calculus
\definecolor{stewartcyan}{RGB}{0, 130, 160}

% Biểu tượng đồ họa CAS chuẩn lề
\newcommand{\calcicon}{%
  \tikz[baseline=-0.55ex, line width=0.5pt]{%
    \draw[stewartcyan, rounded corners=0.8pt] (0,0) rectangle (2.1ex, 1.8ex);
    \draw[stewartcyan, line width=0.4pt] (0.35ex, 0.35ex) -- (1.75ex, 0.35ex);
    \draw[stewartcyan, line width=0.4pt] (0.35ex, 0.35ex) -- (0.35ex, 1.45ex);
    \draw[stewartcyan, line width=0.55pt] (0.35ex, 0.45ex) .. controls (0.85ex, 0.6ex) and (1.25ex, 1.2ex) .. (1.75ex, 1.45ex);
  }%
}
\newcommand{\casicon}{\llap{\calcicon\hspace{4pt}}}

% Định dạng tiêu đề đầu trang theo chuẩn bản gốc
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textsf{\textbf{138}}\qquad\textbf{\textsf{CHƯƠNG 2}}\quad Giới hạn và Đạo hàm}
\renewcommand{\headrulewidth}{0pt}

% Tinh chỉnh khoảng cách công thức toán
