\fancyhead[L]{\fontsize{9pt}{11pt}\selectfont\textbf{\textsf{10}}\qquad \textbf{\textsf{CHƯƠNG 1}}\quad \textsf{Các hàm số và Mô hình}}
\fancyhead[R]{}
...
\begin{minipage}[t]{0.268\textwidth}
    \vspace*{1.3cm}
    \centering
    \includegraphics[width=0.92\linewidth]{images/p0045_fig7_clean.png}\\[2pt]
    {\small\textbf{\textsf{HÌNH 7}}}

    \vspace{1.1cm}
    \includegraphics[width=0.92\linewidth]{images/p0045_fig8_clean.png}\\[2pt]
    {\small\textbf{\textsf{HÌNH 8}}}

    \vspace{0.8cm}
    {\raggedright\fontsize{7.8pt}{10pt}\selectfont\color{darkgray} Biểu thức
    \[
    \frac{f(a+h) - f(a)}{h}
    \]
    trong Ví dụ 3 được gọi là \textbf{thương sai phân} (\textit{difference quotient}) và xuất hiện rất thường xuyên trong giải tích. Như chúng ta sẽ thấy trong Chương 2, nó biểu thị tốc độ biến thiên trung bình của hàm số $f(x)$ giữa $x = a$ và $x = a + h$.\par}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.705\textwidth}
    ...
    \begin{itemize}[itemsep=1pt, topsep=2pt, leftmargin=1.5em]
        \item[{\color{stewartred}\textbullet}] \makebox[3.2cm][l]{\textbf{bằng lời}} (mô tả bằng câu chữ)
        \item[{\color{stewartred}\textbullet}] \makebox[3.2cm][l]{\textbf{bằng số liệu}} (bằng một bảng giá trị)
        \item[{\color{stewartred}\textbullet}] \makebox[3.2cm][l]{\textbf{bằng hình ảnh trực quan}} (bằng đồ thị)
        \item[{\color{stewartred}\textbullet}] \makebox[3.2cm][l]{\textbf{bằng đại số}} (bằng một công thức giải tích tường minh)
    \end{itemize}
    \vspace{2pt}

    Nếu một hàm số có thể được biểu diễn bằng cả bốn phương pháp trên, việc chuyển đổi linh hoạt qua lại giữa các cách biểu diễn sẽ mang lại cái nhìn sâu sắc hơn về bản chất của hàm số đó. (Chẳng hạn, trong Ví dụ 2, chúng ta đã bắt đầu với các công thức đại số rồi sau đó thu được đồ thị.) Tuy nhiên, một số hàm số nhất định lại được mô tả tự nhiên hơn bằng phương pháp này thay vì phương pháp khác. Với tinh thần đó, chúng ta hãy cùng xem xét lại bốn tình huống đã nêu ở phần mở đầu của mục này.
\end{minipage}