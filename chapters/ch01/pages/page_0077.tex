...
\begin{minipage}[t]{0.71\textwidth}
    ...
    \vspace{0.41em}
    \noindent
    Biểu thức này xác định khi cả $2 - x \ge 0$ và $2 - \sqrt{2 - x} \ge 0$. Bất đẳng thức thứ nhất có nghĩa là $x \le 2$, và bất đẳng thức thứ hai tương đương với $\sqrt{2 - x} \le 2$, hay $2 - x \le 4$, tức là $x \ge -2$. Do đó $-2 \le x \le 2$, vì vậy tập xác định của $g \circ g$ là đoạn $[-2, 2]$. \hfill{\color{stewartcyan}$\blacksquare$}
...
\begin{minipage}{\textwidth}
    \begin{tikzpicture}[remember picture, overlay]
        % Các đường kẻ sọc trang trí phía trước
    \end{tikzpicture}
    \noindent
    \begin{tabular}{@{}m{0.18\textwidth}@{\hspace{1em}}m{0.32\textwidth}@{\hspace{1em}}m{0.45\textwidth}@{}}
        \begin{tikzpicture}
            \foreach \y in {0, 2.5, 5, 7.5} {
                \draw[stewartcyan, thick] (0,\y pt) -- (2.5,\y pt);
            }
        \end{tikzpicture}
        &
        {\fontsize{24}{28}\selectfont \textbf{\textsf{\color{stewartcyan}1.3}}\quad \textbf{\textsf{BÀI TẬP}}}
        &
        \begin{tikzpicture}
            \foreach \y in {0, 2.5, 5, 7.5} {
                \draw[stewartcyan, thick] (0,\y pt) -- (\linewidth,\y pt);
            }
        \end{tikzpicture}
    \end{tabular}
\end{minipage}
...
\begin{minipage}[t]{0.48\textwidth}
    \vspace{0pt}
    \begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=1.5em, start=3]
        ...
        \begin{multicols}{2}
        \begin{enumerate}[label=(\alph*), itemsep=3pt, topsep=3pt]
            \item $y = f(x - 4)$
            \item $y = f(x) + 3$
            \item $y = \frac{1}{3}f(x)$
            \item $y = -f(x + 4)$
        ...
        \vspace{0.41em}
        \begin{center}
            \includegraphics[width=0.92\linewidth]{images/p0077_fig1.png}
        \end{center}
    \end{enumerate}
\end{minipage}