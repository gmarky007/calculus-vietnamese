...
\fancyhead[L]{\textbf{\textsf{44}}\qquad \textbf{\textsf{\color{stewartcyan}CHƯƠNG 1}}\quad \textsf{Hàm số và Mô hình}}
\fancyhead[R]{}
...
\smallskip
\noindent
\textbf{34.} $f(x) = \dfrac{1}{x - 1}, \quad g(x) = \dfrac{x}{x - 2}$
...
\textbf{45.} $F(x) = (2x + x^2)^4$ & \textbf{46.} $F(x) = \cos^2 x$ \\[0.8ex]
\textbf{47.} $F(x) = \dfrac{\sqrt[3]{x}}{1 + \sqrt[3]{x}}$ & \textbf{48.} $G(x) = \sqrt[3]{\dfrac{x}{1 + x}}$ \\[0.8ex]
...
\noindent
\begin{tabular*}{\linewidth}{@{}p{0.48\linewidth}@{\extracolsep{\fill}}p{0.48\linewidth}@{}}
\textbf{51.} $R(x) = \sqrt{\sqrt{x} - 1}$ & \textbf{52.} $H(x) = \sqrt[8]{2 + |x|}$ \\[0.8ex]
...
(d) $(g \circ f)(6)$ \hspace{1em} & (e) $(g \circ g)(-2)$ \hspace{1em} & (f) $(f \circ f)(4)$
\end{tabular*}

\smallskip
{\centering
\includegraphics[width=0.52\linewidth]{images/p0079_fig1.png}\par}
\smallskip
...
\smallskip
{\centering
\includegraphics[width=0.52\linewidth]{images/p0079_fig2.png}\par}
\smallskip

\noindent
\textbf{59.} Một viên đá được thả xuống hồ, tạo ra một gợn sóng hình tròn lan rộng ra phía ngoài với tốc độ $60\text{ cm/s}$.
\begin{enumerate}[label=\textbf{(\alph*)},leftmargin=1.5em,itemsep=0pt,topsep=1pt,parsep=0pt,partopsep=0pt]
    \item Biểu diễn bán kính $r$ của đường tròn này theo hàm số của thời gian $t$ (tính bằng giây).
    \item Nếu $A$ là diện tích của hình tròn này dưới dạng hàm số của bán kính, hãy tìm $A \circ r$ và giải thích ý nghĩa của nó.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{(\alph*)},leftmargin=1.5em,itemsep=0pt,topsep=1pt,parsep=0pt,partopsep=0pt]
    ...
    \item Nếu $V$ là thể tích của quả bóng dưới dạng hàm số của bán kính, hãy tìm $V \circ r$ và giải thích ý nghĩa của nó.
\end{enumerate}
