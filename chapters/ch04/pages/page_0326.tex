...
\usepackage[paperwidth=8.5in, paperheight=11in, left=0.6in, right=0.6in, top=0.6in, bottom=0.6in]{geometry}
\usepackage{fontspec}
...
% Màu sắc nhận diện chuẩn giáo trình Stewart Calculus 9th edition
\definecolor{stewartblue}{RGB}{0, 115, 175}
\definecolor{boxred}{RGB}{195, 45, 45}
\definecolor{pspurple}{RGB}{110, 50, 120}

\pagestyle{empty}

% Nhãn số phương trình đóng khung đỏ
\newcommand{\redtag}[1]{%
  \tikz[baseline=(tag.base)]{%
    \node[draw=boxred, line width=0.6pt, inner sep=2pt, font=\footnotesize\bfseries, text=boxred] (tag) {#1};%
  }%
}
